...
def test_prune_shared_vars_only_when_unused_by_all_machines(
    ...
    store = in_repo.VarsStore(flake=flake_obj)
    assert store.exists(
        GeneratorId(name="shared_gen", placement=Shared()), "shared_val"
    )
...
def test_prune_shared_vars_when_no_machine_uses_them(
    ...
    store = in_repo.VarsStore(flake=flake_obj)
    assert not store.exists(
        GeneratorId(name="orphan_shared", placement=Shared()), "val"
    )
...
@pytest.mark.broken_on_darwin
@pytest.mark.with_core
def test_prune_removes_vars_for_machines_no_longer_in_config(
    monkeypatch: pytest.MonkeyPatch,
    flake_with_sops: ClanFlake,
) -> None:
    """Prune (no machine arg) removes the entire per-machine dir for a machine
    that was fully removed from config.
    """
    flake = flake_with_sops

    config_a = flake.machines["machine_a"] = create_test_machine_config()
    gen_a = config_a["clan"]["core"]["vars"]["generators"]["gen_a"]
    gen_a["files"]["val_a"]["secret"] = False
    gen_a["script"] = 'echo -n a > "$out"/val_a'

    config_b = flake.machines["machine_b"] = create_test_machine_config()
    gen_b = config_b["clan"]["core"]["vars"]["generators"]["gen_b"]
    gen_b["files"]["val_b"]["secret"] = False
    gen_b["script"] = 'echo -n b > "$out"/val_b'
    ...
    machine_b_dir = flake.path / "vars" / "per-machine" / "machine_b"
    assert machine_b_dir.exists()
    assert (machine_b_dir / "gen_b" / "val_b").exists()

    # Fully remove machine_b: drop its config and the tracked disk dir
    shutil.rmtree(flake.path / "machines" / "machine_b", ignore_errors=True)
    del flake.machines["machine_b"]
    sp.run(["git", "add", "-A"], cwd=flake.path, check=True)
    sp.run(
        ["git", "commit", "-m", "remove machine_b"],
        cwd=flake.path,
        check=True,
    )
    ...
    assert (
        flake.path / "vars" / "per-machine" / "machine_a" / "gen_a" / "val_a"
    ).exists()


@pytest.mark.broken_on_darwin
@pytest.mark.with_core
def test_prune_does_not_commit_unrelated_staged_changes(
    monkeypatch: pytest.MonkeyPatch,
    flake_with_sops: ClanFlake,
) -> None:
    """Prune's auto-commit must only include its own removed paths, not any
    unrelated changes the user already has staged in the index.
    """
    flake = flake_with_sops
    ...
    # Orphan the generator by dropping it from config
    config = flake.machines["my_machine"] = create_test_machine_config()
    flake.refresh()

    # Stage an unrelated change — simulating user WIP in the index
    unrelated = flake.path / "UNRELATED_STAGED.txt"
    unrelated.write_text("do not commit me\n")
    sp.run(["git", "add", str(unrelated)], cwd=flake.path, check=True)
    staged_before = sp.run(
    ...
    assert "UNRELATED_STAGED.txt" not in commit_files

    # And the unrelated file must still be staged afterwards
    staged_after = sp.run(
        ["git", "diff", "--cached", "--name-only"],
        cwd=flake.path,
        capture_output=True,
    ...
    assert "UNRELATED_STAGED.txt" in staged_after
